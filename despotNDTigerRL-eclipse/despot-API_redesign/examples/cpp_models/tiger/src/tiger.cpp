...
bool Tiger::Step(State& s, double random_num, ACT_TYPE action, double& reward,
	...
	Globals::config.jlog_["current_tigerposition_state(s)"]=(state.tiger_position==LEFT?"LEFT":"RIGHT");
...
double Tiger::ObsProb(OBS_TYPE obs, const State& s, ACT_TYPE a) const {
	...
	if (a != LISTEN){
		Globals::config.jlog_["observation_probability"]=obs==2;
		return obs == 2;
	}

	Globals::config.jlog_["observation_probability"]=state.tiger_position == obs ? (1 - NOISE) : NOISE;

	return state.tiger_position == obs ? (1 - NOISE) : NOISE;
}


void Tiger::GetBelief(const Belief& belief) const{
	...
	for (int i = 0; i < particles.size(); i++) {
		...
		const TigerState* state = static_cast<const TigerState*>(particle);

		statevariable1[state->tiger_position]+=particle->weight;

	}

	// In statevariable1: 1 is right, 0 is left
	Globals::config.current_satisfaction_criteria[0]=statevariable1[0];
	Globals::config.current_satisfaction_criteria[1]=statevariable1[1];
...
void Tiger::PrintState(const State& state, ostream& out) const {
	...
	Globals::config.jlog_["arrived_tigerposition_(s')"]=(tigerstate.tiger_position==LEFT?"LEFT":"RIGHT");
...
void Tiger::PrintObs(const State& state, OBS_TYPE obs, ostream& out) const {
	...
	Globals::config.jlog_["observation_description"]=(obs == LEFT ? "LEFT" : "RIGHT");
