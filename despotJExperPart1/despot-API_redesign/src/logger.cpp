...
bool Logger::SummarizeStep(int step, int round, bool terminal, ACT_TYPE action,
	...
	*out_ << "-----------------------------------Round " << round << " Step "
			<< step << "-----------------------------------" << endl;
	//Globals::config.jlog_["step_nro"]=step;
	...
	*out_ << endl;
	...
	// 18/11/2018
	// relocated logic to get the current belief after finishing the step
	// otherwise we were logging the belief of the previous time slice
	...
	Globals::config.jlog_["acu_rewards_discounted"]=total_discounted_reward_;
	Globals::config.jlog_["acu_rewards_undiscounted"]=total_undiscounted_reward_;
	Globals::config.jlog_["step_reward"]=reward_;
